...
def convert_times_to_milliseconds(time):
    ...
    if time == '16.38.8':
        return minutes_to_millis('16') + seconds_to_millis('38') + hundredths_to_millis('8')
    if time == '30.13.74':
        return minutes_to_millis('30') + seconds_to_millis('13') + hundredths_to_millis('74')
    ...
    # if it's a longer race, but the time is very fast (eg, 58:42 for half marathon)
    if (len(cleaned_time) <= 5) and (':' in cleaned_time):
    ...
    if (len(cleaned_time) > 8) and ('.' in cleaned_time) and (':' in cleaned_time):
        rest, hundredths = cleaned_time.split('.')
        hours, minutes, seconds = rest.split(':')
        return hours_to_millis(hours) + minutes_to_millis(minutes) + seconds_to_millis(seconds) + hundredths_to_millis(hundredths)
    ...
    # which still has hundredths of seconds
        ...
        return minutes_to_millis(minutes) + seconds_to_millis(seconds) + hundredths_to_millis(hundredths)
    ...
    # which does not have hundredths of seconds but does have hours
        ...
        return hours_to_millis(hours) + minutes_to_millis(minutes) + seconds_to_millis(seconds)
    ...
    for event in EVENT_LIST:
        ...
        try:
            ...
            if mens_outdoor_time is not None:
                TIME_DICT[event]['men'][nation] = convert_times_to_milliseconds(mens_outdoor_time)
        ...
        try:
            ...
            if womens_outdoor_time is not None:
                TIME_DICT[event]['women'][nation] = convert_times_to_milliseconds(womens_outdoor_time)
        except IndexError:
